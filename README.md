...
- Public bundle boundary tightened: public static JS avoids Course/Udemy/manual-cookie/cookies wording while keeping backend error-code compatibility. URL downloads expose a separate opt-in `Use Chrome session if the source requires sign-in` control under Advanced save options.
